...
use crate::templates::broker_template_generator::BrokerTemplateGenerator;
...
/// Command that initializes a new local event streaming workspace.
...
impl BaseCommand for InitCommand {
    fn execute(&self) -> Result<CommandResult, CliError> {
        let template_generator =
            WorkspaceTemplateGenerator::new(vec![Box::new(BrokerTemplateGenerator)]);
        let workspace_initializer = WorkspaceInitializer::new(
